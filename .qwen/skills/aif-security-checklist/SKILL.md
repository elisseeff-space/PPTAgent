...
For `/skills aif-security-checklist` audits (full audit or category audit), keep the human-readable security report first and append one final fenced `aif-gate-result` JSON block.
...
- Set `"suggested_next": {` to `/skills aif-fix` for code/config security fixes or `null` when no workflow command fits.

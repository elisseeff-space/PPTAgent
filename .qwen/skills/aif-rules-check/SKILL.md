...
Run a standalone read-only rules gate for project rules. This command checks rule compliance only; it does not replace `/skills aif-review` or `/skills aif-verify`.
...
- Suggest `/skills aif-rules <rule text>` for axioms
- Suggest `/skills aif-rules area:<name>` for area-specific rules
...
- `/skills aif-review` for broader code review
- `/skills aif-verify` for full plan-completeness verification
- `/skills aif-rules` when the underlying rules need to be captured or corrected
...
- Set `"suggested_next": {` to `/skills aif-rules` when rules should be added or clarified, `/skills aif-fix` when code must change, or `null` when no allowed next command fits.
- Do not use `/skills aif-review` in the JSON `suggested_next.command`; it may appear only in human-readable workflow suggestions.

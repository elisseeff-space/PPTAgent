...
If the user wants a standalone rules-only pass, suggest `/skills aif-rules-check`. Keep human context-gate labels at `WARN` / `ERROR`, then derive the final machine-readable gate result from the full verification report.
...
- Set `"suggested_next": {` to `/skills aif-fix`, `/skills aif-rules`, `/skills aif-architecture`, `/skills aif-roadmap`, `/skills aif-commit`, or `null` according to `references/GATE-RESULT-CONTRACT.md`.

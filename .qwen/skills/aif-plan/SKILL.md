...
- Treat `--parallel` as disabled: do not create a worktree and do not auto-invoke `/skills aif-implement`.
...
**Full mode + parallel (`--parallel`):** Automatically invoke `/skills aif-implement` — the whole point of parallel is autonomous end-to-end execution in an isolated worktree. If `HANDOFF_BRANCH_PREPARED = 1`, treat `--parallel` as disabled and do not auto-invoke `/skills aif-implement`.

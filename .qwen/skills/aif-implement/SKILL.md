...
If `$ARGUMENTS` contains `--without-plan`, execute a single scoped task from the description WITHOUT creating or reading any plan file. This is the lightweight path for small `feat`/`chore` tasks that do not justify a full plan but are not bug fixes either (use `/skills aif-fix` for bugs).
...
3. Validation:
   - description is empty →
     ERROR: "Usage: /skills aif-implement --without-plan <description> [--docs=yes|no|warn]"
     → STOP
...
5. Do not add tests by default. Add them only if the description explicitly requests tests (e.g. "with tests", "add tests for X") OR if existing project conventions / touched code paths clearly require them (e.g. a test file mirrors every source file in the area being changed, or a RULES.md / skill-context rule mandates test coverage for this kind of change). When in doubt, prefer NO tests and let the user follow up via `/skills aif-plan` if wider test coverage is needed.
...
- Do NOT invoke `/skills aif-plan` or `/skills aif-fix`.
...
- `--docs=yes` → after completion, show the docs checkpoint (same AskUserQuestion as `Docs: yes` in regular mode) and route changes through `/skills aif-docs`.
...
1. 🔍 /skills aif-verify — Verify the change (recommended)
2. 💾 /skills aif-commit — Commit directly
```
...
Options:
1. Verify first — Run /skills aif-verify (recommended)
...
```
/skills aif-implement --without-plan add GET /healthz endpoint returning {"status":"ok"}
/skills aif-implement --without-plan rename LogLevel.VERBOSE to LogLevel.TRACE --docs=yes
```

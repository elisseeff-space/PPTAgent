...
If the user wants a standalone rules-only pass, suggest `/skills aif-rules-check`. Keep human `/skills aif-review` gate labels at `WARN` / `ERROR`, then append the standard machine-readable gate result with `pass|warn|fail` status.
...
- Set `"suggested_next": {` to `/skills aif-fix`, `/skills aif-rules`, `/skills aif-architecture`, `/skills aif-roadmap`, `/skills aif-commit`, or `null`.
